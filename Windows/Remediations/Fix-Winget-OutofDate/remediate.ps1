...
Start-Transcript -Path "$Env:Programfiles\_MEM\Log\winget.log" -Force -Append
...
& $pwshExecutable -Command {
    ...
    $provider = Get-PackageProvider NuGet -ErrorAction Ignore -PassThru -Verbose
    if (-not $provider) {
    ...
    Install-Module microsoft.winget.client -Force -AllowClobber -Verbose -AcceptLicense -PassThru

    # Import the Microsoft WinGet client module
    Import-Module microsoft.winget.client
    
    # Repair the WinGet package manager
    Repair-WinGetPackageManager -AllUsers -Force -Latest -Verbose -PassThru
}
